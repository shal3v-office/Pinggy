    ...
    try {
        ...
        for(const customer of customers){
            for(const site of customer.sites){
                ...
                var firstOfSite = true;
                for(const link of site.innerLinks){
                    var monitorUrl = url + link;
                    console.log(monitorUrl);
                    axios.get(monitorUrl)
                    .then(function (response) {
                        monitorRows.push(addMonitorRow(response.config.url, response.status, site._id, firstOfSite));
                        firstOfSite = false;
                    })
                    .catch(function (error) {
                        monitorRows.push(addMonitorRow(error.config.url, error.response.status, site._id, firstOfSite));
                        firstOfSite = false;
